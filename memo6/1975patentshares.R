...
# Filter patents after 1975
merged_df_filtered <- merged_df %>%
  filter(format(patent_date, "%Y") > 1975)
...
# Take top 10 large companies from result above
big_companies <- c("SAMSUNG ELECTRONICS CO., LTD.", 
...
ggplot(patent_count_by_company_year, aes(x = year, y = patent_share, color = disambig_assignee_organization, group = disambig_assignee_organization)) +
  geom_line() +
  geom_point() +
  labs(title = "Patent Share by Company (1975-2020)",
       x = "Year",
       y = "Patent Share",
       color = "Company") +
  theme_minimal() +
